...
static unsigned long _clk_vpu_round_rate(struct clk *clk,
					     unsigned long rate)
{
	u32 div;

	div = clk->parent->rate * 2 / rate;
	if ((clk->parent->rate * 2) % rate)
		div++;
	...
	return clk->parent->rate * 2 / div;
}

static int _clk_vpu_set_rate(struct clk *clk, unsigned long rate)
{
	u32 reg;
	u32 div;

	div = clk->parent->rate * 2 / rate;
	if (div > 67 || div < 4 || ((clk->parent->rate * 2 / div) != rate)) {
		return -EINVAL;
	}
	div -= 4;

	reg = __raw_readl(CCM_PCDR0) & ~0x0000FC00;
	reg |= div << 10;
...
static struct clk vpu_clk = {
	.name = "vpu_clk",
	.parent = &mpll_main_clk[1],
	.round_rate = _clk_vpu_round_rate,
	.set_rate = _clk_vpu_set_rate,
