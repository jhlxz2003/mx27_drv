...
#if defined(CONFIG_FB_MXC_EPSON_L4F0024) || defined(CONFIG_FB_MXC_SAMSUNG_LTV350QV)
...
#if defined(CONFIG_FB_MXC_EPSON_L4F0024) || defined(CONFIG_FB_MXC_SAMSUNG_LTV350QV)
struct l4f00242t_data {
	struct spi_device *spi;
};

static struct l4f00242t_data l4f00242t_lcd;
#endif
 ...
 * while ch7024 supports VGA output.
 */
...
struct mx2fb_tvout_if mx2fb_tvout_func;
#endif
...
struct mx2fb_info mx2fbi_tvout = {
	.type = MX2FB_TYPE_BG,
	.id = "DISP0 BG",
	.registered = 0,
};
...
#define COLMOD_565 0x50
#define COLMOD_666 0x60

#define MADCTL_CASE1 0x00
#define MADCTL_CASE2 0xD0
#define MADCTL_CASE3 0xB0
#define MADCTL_CASE4 0x60
...
{
	l4f00242t_lcd.spi = spi;
#ifdef CONFIG_FB_MXC_EPSON_L4F0024
	spi->mode = SPI_MODE_2;	/* pha = 0 pol = 1 */
	spi->bits_per_word = 9;
#else				/* CONFIG_FB_MXC_SAMSUNG_LTV350QV */
	spi->mode = SPI_MODE_1;	/* pha = 1 pol = 0 */
...
/*!
 * spi driver structure for L4F00242T LCD
 */
static struct spi_driver l4f00242t_spi_driver = {
	.driver = {
		   .name = "lcd_spi",
		   .bus = &spi_bus_type,
		   .owner = THIS_MODULE,
...
/*!
 * This function is called to transfer data to L4F00242T LCD on SPI.
 *
 * @param    spi        the SPI slave device
...
static inline int spi_rw(struct spi_device *spi, u8 * buf, size_t len)
{
	...
	if (spi_sync(spi, &m) != 0 || m.status != 0) {
		printk(KERN_INFO "spi_sync - fail\n");
		return -1;
	}
...
/*!
 * L4F0024 Epson spi send command
 * @param  cmd the command to be writeen to LCD
 */
static void l4f00242t_send_cmd(u8 cmd)
{
	unsigned int rcmd = 0;
	ssize_t ret;
	...
	/* send command */
	ret = spi_rw(l4f00242t_lcd.spi, (u8 *) & rcmd, 1);
	if (ret != 0) {
		printk("CSPI1 Send failed: %d\n", ret);
	}
	/* delay due to the spec */
	mdelay(100);
}

/*!
 * SAMSUNG LTV350QV spi send command
 * @param  cmd the data to be written to LCD
 */
static void ltv350qv_send(u32 data)
{
	unsigned int rcmd = 0;
	ssize_t ret;
...
/*!
 * L4F0024 Epson spi send command and parameter
 *
 * @param   cmd    the command to be sent to LCD
 * @param   param  the command parameter to be sent to LCD
 * @param   len    the parameter length
 *
 */
static void l4f00242t_send_cmd_param(u8 cmd, u8 * param, u8 len)
{
	unsigned int rcmd = 0, rparam = 0;
	ssize_t ret;
	int i;
	...
	ret = spi_rw(l4f00242t_lcd.spi, (u8 *) & rcmd, 1);
	if (ret != 0) {
		printk(KERN_ERR "CSPI1 Send failed: %d\n", ret);
	}

	/* send parameter */
	for (i = 0; i < len; i++) {
		/* parameter input */
		rparam = 0x100 | param[i];
		ret = spi_rw(l4f00242t_lcd.spi, (u8 *) & rparam, 1);
		if (ret != 0) {
			printk(KERN_ERR "CSPI1 Send failed: %d\n", ret);
		}
	}
...
/*
 * Send Power On commands by CSPI1 to L4F00242T03
 *
 */
static void spi_lcd_poweron(void)
{
#ifdef CONFIG_FB_MXC_EPSON_L4F0024
	u8 madctl_data[1] = { MADCTL_CASE1 }, colmod_data[1] = {
	COLMOD_565};

	/* send powe up command */
	mdelay(20);
	l4f00242t_send_cmd_param(MADCTL, madctl_data, 1);
	l4f00242t_send_cmd_param(COLMOD, colmod_data, 1);
	l4f00242t_send_cmd(SLPOUT);	/* SLPOUT */
	l4f00242t_send_cmd(DISON);	/* DISON */
#else				/* CONFIG_FB_MXC_SAMSUNG_LTV350QV */
	ltv350qv_send(0x00740009);
	ltv350qv_send(0x00760000);

	mdelay(15);
	ltv350qv_send(0x00740009);
	ltv350qv_send(0x00764000);

	ltv350qv_send(0x0074000A);
	ltv350qv_send(0x00762000);

	ltv350qv_send(0x00740009);
	ltv350qv_send(0x00764055);

	mdelay(55);
	ltv350qv_send(0x00740001);
	ltv350qv_send(0x0076409D);

	ltv350qv_send(0x00740002);
	ltv350qv_send(0x00760204);

	ltv350qv_send(0x00740003);
	ltv350qv_send(0x00760100);

	ltv350qv_send(0x00740004);
	ltv350qv_send(0x00763000);

	ltv350qv_send(0x00740005);
	ltv350qv_send(0x00764003);

	ltv350qv_send(0x00740006);
	ltv350qv_send(0x0076000A);

	ltv350qv_send(0x00740007);
	ltv350qv_send(0x00760021);

	ltv350qv_send(0x00740008);
	ltv350qv_send(0x00760C00);

	ltv350qv_send(0x00740010);
	ltv350qv_send(0x00760103);

	ltv350qv_send(0x00740011);
	ltv350qv_send(0x00760301);

	ltv350qv_send(0x00740012);
	ltv350qv_send(0x00761F0F);

	ltv350qv_send(0x00740013);
	ltv350qv_send(0x00761F0F);

	ltv350qv_send(0x00740014);
	ltv350qv_send(0x00760707);

	ltv350qv_send(0x00740015);
	ltv350qv_send(0x00760307);

	ltv350qv_send(0x00740016);
	ltv350qv_send(0x00760707);

	ltv350qv_send(0x00740017);
	ltv350qv_send(0x00760000);

	ltv350qv_send(0x00740018);
	ltv350qv_send(0x00760004);

	ltv350qv_send(0x00740019);
	ltv350qv_send(0x00760000);

	mdelay(20);
	ltv350qv_send(0x00740009);
	ltv350qv_send(0x00764A55);

	ltv350qv_send(0x00740005);
	ltv350qv_send(0x00765003);
#endif
}

/*
 * Send Power Off commands by CSPI1 to L4F00242T03
 *
 */
static void spi_lcd_poweroff(void)
{
#ifdef CONFIG_FB_MXC_EPSON_L4F0024
	/* send power off command */
	l4f00242t_send_cmd(DISOFF);	/* DISOFF */
	l4f00242t_send_cmd(SLPIN);	/* SLPIN */
#else				/* CONFIG_FB_MXC_SAMSUNG_LTV350QV */
	ltv350qv_send(0x00740009);
	ltv350qv_send(0x00764055);
...
#endif				/* defined(CONFIG_FB_MXC_EPSON_L4F0024) || defined(CONFIG_FB_MXC_SAMSUNG_LTV350QV) */

static void mx2fb_show_logo(struct fb_info *fbi)
{
	/* prepare logo */
		       ...
		       unsigned long arg)
{
...
#endif		
		g_gwinfo.ck_red = (ck.color_key & 0x00FF0000) >> 19;
		g_gwinfo.ck_green = (ck.color_key & 0x0000FF00) >> 10;
		g_gwinfo.ck_blue = (ck.color_key & 0x000000FF) >> 3;
	...
	case ENCODER_GET_CAPABILITIES:{
			...
			if (mx2fb_tvout_func.get_capability != NULL)
				mx2fb_tvout_func.get_capability(&cap);
			else
				return -ENODEV;
	...
	case ENCODER_SET_INPUT:{
			...
			mx2fb_tvout_func.set_input(&varg);
			break;
		}
	case ENCODER_SET_OUTPUT:{
			...
			mx2fb_tvout_func.set_output(&varg);
			break;
		}
	case ENCODER_ENABLE_OUTPUT:{
			...
			//mx2fb_disable_lcd();
			mx2fb_tvout_func.enable_output(&varg);
			break;
		}
#endif
	default:
...
//////////////////////////////
//  new add on 2011.08.16   //
//////////////////////////////
#ifdef CONFIG_FB_MXC_SAMSUNG_LTP700WVF01
static const struct fb_videomode *defmode = &mxcfb_modedb[9];
#else
static const struct fb_videomode *defmode = NULL;
#endif
			      ...
			      struct platform_device *pdev)
{
	...
	if (!fb_find_mode(&info->var, info, fb_mode, mxcfb_modedb,
			  mxcfb_modedb_sz, defmode, default_bpp)) {
...
#if defined(CONFIG_FB_MXC_EPSON_L4F0024) || defined(CONFIG_FB_MXC_SAMSUNG_LTV350QV)
		if (mx2fb_tvout_func.enable == NULL)
			lcd_poweron();
#endif
		clk_enable(lcdc_clk);
//		if (mx2fb_tvout_func.enable == NULL)
		{
			printk(KERN_ALERT "mx2fb_tvout_func.enable == NULL and wil call gpio_lcdc_active()\r\n");
			gpio_lcdc_active();
		}
		board_power_lcd(1);
		_set_brightness(255-brightness);
#if defined(CONFIG_FB_MXC_EPSON_L4F0024) || defined(CONFIG_FB_MXC_SAMSUNG_LTV350QV)
		if (mx2fb_tvout_func.enable == NULL)
			spi_lcd_poweron();
#endif
...
//			gpio_lcdc_inactive();
			board_power_lcd(0);
//			_set_brightness(0);  //should be swapped with gpio_lcd_inactive().
			_set_brightness(255);  //should be swapped with gpio_lcd_inactive().
			gpio_lcdc_inactive();  //new add on 2012.12.07
...
//	printk(KERN_ALERT "--- Graphic window enabled.---\n");
}
...
//	printk(KERN_ALERT "--- perclk3 = %lu, pixclock = %lu ---\n", perclk3, var->pixclock);
	/* Calculate pixel clock divider, and round to the nearest integer */
	pcd = (perclk3 * 8 / (PICOS2KHZ(var->pixclock) * 1000UL) + 4) / 8;
//	printk(KERN_ALERT "--- DCLK = %luKHZ, pcd = %d ---\n", PICOS2KHZ(var->pixclock), pcd);
	if (--pcd > 0x3F)
		pcd = 0x3F;

	/* Panel configuration register */
	pcr = 0xFA008B80 | pcd;
	pcr |= (var->sync & FB_SYNC_CLK_INVERT) ? 0x00200000 : 0;
	pcr |= (var->sync & FB_SYNC_SHARP_MODE) ? 0x00000040 : 0;
	pcr |= (var->sync & FB_SYNC_OE_ACT_HIGH) ? 0 : 0x00100000;
	pcr |= (var->sync & FB_SYNC_VERT_HIGH_ACT) ? 0 : 0x00800000;
	pcr |= (var->sync & FB_SYNC_HOR_HIGH_ACT) ? 0 : 0x00400000;
...
//	__raw_writel(0x00A90300 | level, LCDC_REG(LCDC_LPCCR));
	__raw_writel(0x00A90100 | level, LCDC_REG(LCDC_LPCCR));
}
...
#if defined(CONFIG_FB_MXC_EPSON_L4F0024) || defined(CONFIG_FB_MXC_SAMSUNG_LTV350QV)
	spi_register_driver(&l4f00242t_spi_driver);
#endif
...
#if defined(CONFIG_FB_MXC_EPSON_L4F0024) || defined(CONFIG_FB_MXC_SAMSUNG_LTV350QV)
	spi_unregister_driver(&l4f00242t_spi_driver);
#endif
}
