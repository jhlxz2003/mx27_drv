...
static int set_ch1_addr(emma_prp_cfg * cfg, cam_data * cam)
{
	if (cam->rotation != V4L2_MXC_ROTATE_NONE) {
		...
		if (cam->v4l2_fb.flags != V4L2_FBUF_FLAG_OVERLAY) {
			...
			if (g_vaddr_fb)
				iounmap(g_vaddr_fb);
			g_vaddr_fb = ioremap_cached(fb->fix.smem_start, fb->fix.smem_len);
			if (!g_vaddr_fb)
				return -1;
		}
		...
		cfg->ch1_ptr += cam->win.w.top * fb->var.xres_virtual * (fb->var.bits_per_pixel >> 3)
...
//		if (status & PRP_INTRSTAT_CH2OVF) {
	...
	if (cam->overlay_on && (status & (PRP_INTRSTAT_CH1BUF1 | PRP_INTRSTAT_CH1BUF2))) {
		if (cam->rotation != V4L2_MXC_ROTATE_NONE) {
...
static int prp_enc_enable(void *private)
{
	...
	prphw_enable(cam->overlay_on ? (PRP_CHANNEL_1 | PRP_CHANNEL_2): PRP_CHANNEL_2);
...
static int prp_vf_mem_alloc(cam_data * cam)
{
	...
	for (i = 0; i < 2; i++) {
		...
		g_vaddr_vfbuf[i] = ioremap_cached(cam->vf_bufs[i], cam->vf_bufs_size[i]);
		if (!g_vaddr_vfbuf[i]) {
...
static int prp_rot_mem_alloc(cam_data * cam)
{
	...
	for (i = 0; i < 2; i++) {
		cam->rot_vf_buf_size[i] = cam->win.w.width * cam->win.w.height * 2;
		cam->rot_vf_bufs_vaddr[i] = dma_alloc_coherent(0, cam->rot_vf_buf_size[i],
				       &cam->rot_vf_bufs[i],
				       GFP_DMA | GFP_KERNEL);
		...
		g_vaddr_rotbuf[i] = ioremap_cached(cam->rot_vf_bufs[i], cam->rot_vf_buf_size[i]);
		if (!g_vaddr_rotbuf[i]) {
			pr_debug("Failed to ioremap_cached() for rotation buffer.\n");
			prp_rot_mem_free(cam);
			return -1;
		}
	}
...
static int prp_vf_start(void *private)
{
	...
	prphw_enable(cam->capture_on ? (PRP_CHANNEL_1 | PRP_CHANNEL_2) : PRP_CHANNEL_1);
...
static void deinterlace_u16(u8 *src, int s_stride, int width, int height, u8 *dst)
{
	u8 *sptr0 = src;
	u8 *dptr = dst;
	u8 *sptr1;
	...
	sptr1 = src + width*height;
	for (i = 0; i < n; ++i) {
		memcpy(dptr, sptr0, s_stride);
		dptr += s_stride;
		sptr0 += s_stride;
		memcpy(dptr, sptr1, s_stride);
		dptr += s_stride;
		sptr1 += s_stride;
	}
}
...
static void rotation(unsigned long private)
{
	...
	if (cam->v4l2_fb.flags == V4L2_FBUF_FLAG_OVERLAY) {
		...
		if (!fb)
			return;
		dst = g_vaddr_fb;
		dst += cam->win.w.top * fb->var.xres_virtual * (fb->var.bits_per_pixel >> 3)
		    + cam->win.w.left * (fb->var.bits_per_pixel >> 3);
	...
	case V4L2_MXC_ROTATE_90_RIGHT_VFLIP:
		opl_rotate90_vmirror_u16(src, s_stride, width, height, dst, d_stride);
		break;
	case V4L2_MXC_ROTATE_90_RIGHT_HFLIP:
		/* ROTATE_90_RIGHT_HFLIP = ROTATE_270_RIGHT_VFLIP */
		opl_rotate270_vmirror_u16(src, s_stride, width, height, dst, d_stride);
		break;
	case V4L2_MXC_ROTATE_90_LEFT:
		opl_rotate270_u16(src, s_stride, width, height, dst, d_stride);
		break;
	case V4L2_MXC_DEINTERLACE:
		deinterlace_u16(src, s_stride, width, height, dst);
		break;
	default:
		return;
	}
...
static int prp_resize_check_ch2(emma_prp_cfg * cfg)
{
	...
	h = check_simple(pscale + 1, cfg->in_height, cfg->ch2_height, PRP_CHANNEL_2);
	if ((w != -1) && (h != -1)) {
		pr_debug("Ch2 resize.\n");
		pr_debug("Original width = %d internel width = %d\n",
			 cfg->ch2_width, w);
