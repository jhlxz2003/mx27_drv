...
#ifndef CONFIG_MACH_MX27_MDK27V0
/*!
...
static struct mxcmci_host *mxc_mci_hosts[2] = { NULL, NULL };
extern void sdhc_irqchip_handle(unsigned int irq);
extern int sdhc_write_protect(int id);
extern void sdhc_setup_irq(unsigned int irq, struct mmc_host *mmc);
#endif
...
#ifdef CONFIG_MACH_MX27_MDK27V0
		sdhc_irqchip_handle(host->mmc->sdio_irq);
#else
...
{
	struct mxcmci_host *host = mmc_priv(mmc);
	return sdhc_write_protect(host->id);
}
#endif
...
#ifdef CONFIG_MACH_MX27_MDK27V0
struct mxcmci_host *mxc_get_mmc_host(int id)
{
	if (id < 0 || id > 1)
		return NULL;
	return mxc_mci_hosts[id];
}

EXPORT_SYMBOL(mxc_get_mmc_host);
#endif
...
static int mxcmci_resume(struct platform_device *pdev)
{
	...
	if (!host->irq) {
		ret = -ENOMEM;
		printk(KERN_INFO "%s:host->irq get error\n", __func__);
		return ret;
	}
#ifdef CONFIG_MACH_MX27_MDK27V0
#ifdef CONFIG_MACH_MX27_TUOSI
	...
	host->detect_irq = platform_get_irq(pdev, 1);
	if (!host->detect_irq) {
		printk(KERN_INFO "%s:host->detect_irq get error\n", __func__);
		return -1;
	}
	...
	ret =
	    request_irq(host->detect_irq, mxcmci_gpio_irq, 0, pdev->name, host);
	if (ret) {
		printk(KERN_INFO "%s:request detect_irq error:%d\n", __func__, ret);
		return ret;
	}
	...
	ret = request_irq(host->irq, mxcmci_irq, 0, pdev->name, host);
	if (ret) {
		printk(KERN_INFO "%s:request host->irq error:%d\n", __func__, ret);
		return ret;
	}
